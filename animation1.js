...
function showBlock(){
    ...
    ctx.clearRect(0, 0, gameAreaW, gameAreaH);
    ctx.drawImage(backGroundImg, 0, 0, gameAreaW, gameAreaH);
    for(var i = 0; i < 16 ; i++){
        for(var j = 0 ; j < 10 ; j++){
            if(gridArr[i][j]){

                ctx.drawImage(BlockColorArr[ gridArr[i][j].shape -1], j*43*scaleW + 2*scaleW , i*43*scaleH + 2*scaleH, 40*scaleW, 40*scaleH);
            }
        }
    }
